...
/**
 * Click an element specified by gid or visible text.
 * The function retries with multiple selector strategies, including a
 * case-insensitive text search, and throws an error containing all
 * attempted selectors if the element cannot be located.
 *
 * @param context - Browser context containing the page.
 * @param text - Visible text of the element.
 * @param gid - Unique element identifier generated by {@link tagAllElementsOnPage}.
 * @param tagName - Optional tag name of the target element (defaults to `a`).
 */
export async function clickElement(context: AgentBrowserContext, { text, gid, tagName }: { text: string, gid: number, tagName?: string }) {
    ...
    if (gid) {
        ...
        selectorsTried.push(linkHtmlSelector);
    ...
    if (!elementFoundAndClicked && text) {
        const textSelector = `${tagName} ::-p-text(${text})`;
        selectorsTried.push(textSelector);
        const link = await page.$(textSelector);
        if (link) {
            await betterClick(page, link);
            elementFoundAndClicked = true;
        }
    ...
    if (!elementFoundAndClicked && text) {
        ...
        const xpath = `//${tagName}[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), "${lower}")]`;
        selectorsTried.push(`XPath:${xpath}`);
        const [link] = await page.$x(xpath);
        if (link) {
            await betterClick(page, link as ElementHandle);
            elementFoundAndClicked = true;
        }
    }

    if (!elementFoundAndClicked) {
        webAgentLog.error(`Cannot find link with text ${text} or gid ${gid} on ${page.url()}`, { selectorsTried });
        throw new Error(`Element not found for selectors: ${selectorsTried.join(', ')}`);
    }
