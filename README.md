...
After building the project you can sign any message with an RSA private key using:

```bash
npm run build
node dist/sign_cli.js --key path/to/private.pem --message "your text"
```

The CLI outputs a base64 signature that can be used in external APIs.

## Using the verify-cli

You can verify a signature generated by `sign-cli` with a public key:

```bash
npm run build
node dist/verify_cli.js --key path/to/public.pem --message "your text" --signature <base64>
```
